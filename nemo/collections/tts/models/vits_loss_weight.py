...
from nemo.collections.tts.losses.vits_losses import DiscriminatorLoss
from nemo.collections.tts.losses.vits_losses_loss_weight import FeatureMatchingLoss, GeneratorLoss, KlLoss
...
from nemo.collections.tts.torch.tts_data_types import LossWeight, SpeakerID
...
class VitsModel(TextToWaveform):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (
            ...
            and LossWeight in self._train_dl.dataset.sup_data_types_set
        ):
            (audio, audio_len, text, text_len, speakers, loss_weight) = batch
        elif SpeakerID in self._train_dl.dataset.sup_data_types_set:
            (audio, audio_len, text, text_len, speakers) = batch
        elif LossWeight in self._train_dl.dataset.sup_data_types_set:
            (audio, audio_len, text, text_len, loss_weight) = batch
        else:
        ...
        with autocast(enabled=False):  # TODO: maybe disc need loss_weight?
        ...
        with autocast(enabled=False):
            loss_dur = torch.sum(
                l_length.float().transpose(0, -1) * loss_weight
            )  # duration maybe not need loss_weight
            loss_mel = (
                F.l1_loss(audio_mel, audio_pred_mel, reduction="none").transpose(0, -1) * loss_weight
            ).mean() * self._cfg.c_mel
            loss_kl = (
                self.kl_loss(z_p=z_p, logs_q=logs_q, m_p=m_p, logs_p=logs_p, z_mask=z_mask, loss_weight=loss_weight)
                * self._cfg.c_kl
            )
            loss_fm = self.feat_matching_loss(fmap_r=fmap_r, fmap_g=fmap_g, loss_weight=loss_weight)
            loss_gen, losses_gen = self.gen_loss(disc_outputs=y_d_hat_g, loss_weight=loss_weight)
