...
from nemo.collections.tts.losses.fastpitchloss_loss_weight import DurationLoss, EnergyLoss, MelLoss, PitchLoss
...
class FastPitchModel(SpectrogramGenerator, Exportable, FastPitchAdapterModelMixin):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.learn_alignment:
            # TODO: change this dataset class to get loss_weight
            ...
            loss_weight = batch_dict.get("loss_weight", None)
        else:
            audio, audio_lens, text, text_lens, durs, pitch, speaker, loss_weight = batch
        ...
        # TODO: maybe can't directly use loss_weight

        mel_loss = self.mel_loss_fn(spect_predicted=mels_pred, spect_tgt=mels, loss_weight=loss_weight)
        dur_loss = self.duration_loss_fn(
            log_durs_predicted=log_durs_pred, durs_tgt=durs, len=text_lens, loss_weight=loss_weight,
        )
        loss = mel_loss + dur_loss
        if self.learn_alignment:
            ctc_loss = self.forward_sum_loss_fn(
                attn_logprob=attn_logprob, in_lens=text_lens, out_lens=spec_len, loss_weight=loss_weight,
            )
            bin_loss_weight = min(self.current_epoch / self.bin_loss_warmup_epochs, 1.0) * 1.0
            bin_loss = (
                self.bin_loss_fn(hard_attention=attn_hard, soft_attention=attn_soft, loss_weight=loss_weight,)
                * bin_loss_weight
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        pitch_loss = self.pitch_loss_fn(
            pitch_predicted=pitch_pred, pitch_tgt=pitch, len=text_lens, loss_weight=loss_weight,
        )
        energy_loss = self.energy_loss_fn(
            energy_predicted=energy_pred, energy_tgt=energy_tgt, length=text_lens, loss_weight=loss_weight,
        )
