...
from nemo.core.neural_types.elements import FloatType, LengthsType, LogprobsType, LossType, ProbsType
...
class ForwardSumLoss(Loss):
    def __init__(self, blank_logprob=-1, loss_scale=1.0):
        ...
        self.ctc_loss = torch.nn.CTCLoss(reduction="none", zero_infinity=True)
    ...
    @typecheck()
    def forward(self, attn_logprob, in_lens, out_lens, loss_weight):
        key_lens = in_lens
        query_lens = out_lens
        ...
        cost = (cost * loss_weight / key_lens).sum() / key_lens.shape[0]
...
class BinLoss(Loss):
    ...
    @typecheck()
    def forward(self, hard_attention, soft_attention, loss_weight):
        log_sum = (torch.log(torch.clamp(soft_attention[hard_attention == 1], min=1e-12)) * loss_weight).sum()
